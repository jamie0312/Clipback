...
python scripts/augment_qwen_final.py
...
| 파일 | 설명 | 링크 |
|---|---|---|
| `lora_weights_aug2/` | LoRA 가중치 (최종) | GitHub에 포함 |
| `embeddings_lora/` | FAISS 인덱스 | GitHub에 포함 |
| `data/items.json` | 분실물 메타데이터 | GitHub에 포함 |
| `data/images/` | 분실물 이미지 1336개 | 별도 다운로드 필요 |
 
> 이미지 파일은 용량 문제로 GitHub에 포함되지 않습니다.
> 직접 수집하려면 `scripts/download_images.py`를 실행하세요.
